...
const Slide: React.FC<{
  ...
  return (
    <AbsoluteFill className="items-center justify-center bg-slate-950 text-white">
      <div
        className="flex w-[1000px] flex-col items-center text-center"
        style={{ opacity: enter, transform: `translateY(${y}px)` }}
      >
        {kicker ? (
          <div
            className="mb-4 text-2xl font-semibold tracking-widest uppercase"
            style={{ color: BRAND }}
          >
            {kicker}
          </div>
        ) : null}
        <div className="text-6xl leading-tight font-bold">{title}</div>
        {children ? (
          <div className="mt-8 text-3xl text-slate-300">{children}</div>
        ) : null}
      </div>
    </AbsoluteFill>
  );
...
const SceneTitle: React.FC = () => {
  ...
  return (
    <AbsoluteFill className="items-center justify-center bg-slate-950 text-white">
      ...
      >
        <div className="text-8xl font-black">Remotion</div>
        <div className="mt-6 text-4xl text-slate-300">って何？</div>
      </div>
    </AbsoluteFill>
  );
};
...
const SceneFrame: React.FC = () => {
  ...
  return (
    <AbsoluteFill className="items-center justify-center bg-slate-950 text-white">
      ...
      >
        <div
          className="mb-6 text-2xl font-semibold tracking-widest uppercase"
          style={{ color: BRAND }}
        >
          仕組み
        </div>
        <div className="text-5xl leading-tight font-bold">
          いま何フレーム目か を React に渡すと
        </div>
        <div className="mt-10 font-mono text-3xl text-slate-400">
          useCurrentFrame()
        </div>
        <div
          className="mt-4 font-mono text-8xl font-black tabular-nums"
          style={{ color: BRAND }}
        >
          {(frame < 10 ? "00" : frame < 100 ? "0" : "") + frame}
        </div>
        <div className="mt-6 text-3xl text-slate-300">
          に応じて画面が描き変わる
        </div>
      </div>
    </AbsoluteFill>
  ...
  return (
    <AbsoluteFill className="items-center justify-center bg-slate-950 text-white">
      ...
      >
        ...
        <div className="mt-12 h-8 w-[800px] overflow-hidden rounded-full bg-slate-800">
          <div
            className="h-full rounded-full"
            style={{ width: `${progress * 100}%`, backgroundColor: BRAND }}
          />
        </div>
        <div className="mt-5 font-mono text-3xl tabular-nums text-slate-300">
...
const SceneCode: React.FC = () => {
  ...
  return (
    <AbsoluteFill className="items-center justify-center bg-slate-950 text-white">
      <div
        className="flex w-[1000px] flex-col items-center"
        style={{ opacity: enter }}
      >
        <div className="mb-8 text-center text-5xl font-bold">
          書いたコードが、そのまま動画に
        </div>
        <div className="w-[820px] rounded-2xl bg-slate-900 p-8 text-left shadow-2xl ring-1 ring-slate-700">
          {lines.map((line, i) => (
            <div
              key={i}
              className="font-mono text-2xl leading-relaxed text-slate-200"
            >
              {line || " "}
            </div>
          ))}
...
const SceneOutro: React.FC = () => {
  ...
  return (
    <AbsoluteFill className="items-center justify-center bg-slate-950 text-white">
      ...
      >
        <div className="text-7xl font-black">コードで、動画を作ろう。</div>
        <div className="mt-6 text-3xl font-semibold" style={{ color: BRAND }}>
          Remotion
        </div>
      </div>
    </AbsoluteFill>
